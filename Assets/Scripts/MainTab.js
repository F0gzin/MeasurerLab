...
ConfirmAnaliseButton.addEventListener("click",function(ev){
    ...
    if(! (isNaN(temp)||isNaN(dens))){
        ...
        if (type == 'etanol') {
            ...
            teorValue = etanolTable[tempStr]?.[densStr];

            if (teorValue) {
                const teorNum = parseFloat(teorValue);
                const minTeor = 92.70;
                const maxTeor = 95.00;
                const valid = teorNum >= minTeor && teorNum <= maxTeor;

                teor = `<span style="color:${valid ? '#269f16ff' : '#E53E3E'};">(${valid ? '✔ Conforme!' : '✖ Fora da medida'})</span>`;
                ok = ok && valid;
            } else {
